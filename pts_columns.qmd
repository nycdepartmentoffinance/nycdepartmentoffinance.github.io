...
table_data <- readr::read_csv("tables/pts_columns.csv") %>% 
    select(-1, -NOTES, -OWNER)

```

The table below includes a list of columns from core tables available in the `PTS` tables within the `FDW` database (`FDW3NF`). Use the search bar below to look for columns within a particular table (e.g. `ASMT`) or by column name (e.g. `CNS_PID`) to see which tables it appears in.
